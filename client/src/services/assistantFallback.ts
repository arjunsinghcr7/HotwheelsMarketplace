...
function superlative(t: string): string | null {
  ...
  if (/pric\w*est|most expensive|highest[\s-]?price|costliest|dearest/.test(t)) return top([...CATALOG].sort((a, b) => b.price - a.price), 'most expensive');
  if (/cheap\w*st|least expensive|lowest[\s-]?price|most affordable/.test(t)) return top([...CATALOG].sort((a, b) => a.price - b.price), 'most affordable');
  if (/rar(e|i)*st|most rare/.test(t)) return top([...CATALOG].sort((a, b) => (b.rarityLevel === 'Super Treasure Hunt' ? 1 : 0) - (a.rarityLevel === 'Super Treasure Hunt' ? 1 : 0) || (b.demandScore ?? 0) - (a.demandScore ?? 0)), 'rarest');
